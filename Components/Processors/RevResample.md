# Rev Resample
The Rev Resample component converts a uniformly time-sampled signal into signal that
is uniformly sampled with respect to angular position.

Rotating systems often exhibit phenomena that are coupled to shaft rotation,
but data acquisition systems typically sample uniformly in time rather than at fixed
angular increments. Because of this, shaft-coupled events can be smeared in the
time domain and can be difficult to interpret, especially if shaft speed is changing
during the event.

By resampling into the angular domain, the relationship between system phenomena
and system rotation are made far clearer. This transformation also facilitates
downstream processing like order tracking and phase analysis.

### Rev Resample Settings

- PPR (Pulses per Revolution)
    - Number of pulses expected in a single revolution of the system
    - Default: 1.0
- Angular Resolution
    - Degrees per sample in the angular domain after resampling
    - Default: 0.087890625 (360°/4096 samples)
