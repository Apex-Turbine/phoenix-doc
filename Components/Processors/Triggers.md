## Triggers
The Triggers processor accepts an analog trigger stream as input and produces a
stream of trigger event timestamps. The trigger signal must be characterized by
a few simple parameters that are then used internally to determine a suitable trigger level
and hysteresis for detecting trigger events.
...
- Trigger Type
  ...
  - Visible as a setting, but locked to "Analog" because the goal of this
    component is to detect triggers from an analog signal.
